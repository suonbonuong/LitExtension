...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext = self.select_multiple_data_connector(categories_ext)

		language_ids = duplicate_field_value_from_list(categories_ext['data']['categories_description'], 'language_id')
		...
		categories_ext_queries = self.select_multiple_data_connector(categories_ext_queries)
		categories_ext = self.sync_connector_object(categories_ext, categories_ext_queries)
	...
	def convert_category_export(self, category, categories_ext):
		...
		category_data['categories_id'] = category['categories_id']
		...
		category_description = get_row_from_list_by_field(categories_ext['data']['categories_description'],
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'products_images': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_ProductImages WHERE products_id IN  " + product_id_con + " ORDER BY disp_order",
			},
			'products_meta': {
				'type': "select",
		...
		#	'images': {
		#		'type': 'select',
		#		'query': "SELECT * FROM  _DBPRF_images WHERE image_id IN " + image_id_con,
		#	},
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['sku'] = product['products_cost']
		product_data['price'] = product['products_upc_code']
		...
		product_data['description'] = html.unescape(products_ext_data['products_meta'][0]['products_description'])
		...
		#	meta_keywords=get_row_from_list_by_field(product_description,'products_name',1)
		...
		product_data['thumb_image']['url'] = product['products_image']
		product_data['thumb_image']['path'] = "http://localhost/customcart/" + str(product['products_image'])


		#product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'product_id',products['product_id'])
		#if product_images:
		#	if check_thumbnail:
			...
			for address_book in address_books:
				...
				if state_id:
					...
					if state:
						...
						address_data['state']['state_code'] = state_id
				else:
	...
	def get_orders_ext_export(self, orders):
		...
		cus_ids=duplicate_field_value_from_list(orders['data'], 'customers_id')
		orders_ext_queries = {
			'order_items': {
				'type': 'select',
			...
			'orders_customer': {
				'type': 'select',
				'query': "SELECT * FROM ` _DBPRF_CustomerAddresses` WHERE customers_id IN " + self.list_to_in_condition(cus_ids)
			},
				'orders_payment': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_OrderPayments WHERE orders_id  IN " + self.list_to_in_condition(order_ids)
			},
			...
			'zones': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_states WHERE zones_id IN " + self.list_to_in_condition(state_ids)
			}
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name',
		...
		billing_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name',
